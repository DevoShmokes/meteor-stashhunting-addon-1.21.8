            ...
            if (!chunk.equals(prevChunk) || !chunk.countsEqual(prevChunk)) {
                if (sendNotifications.get())
                {
                    switch (notificationMode.get())
                    {
                        ...
                        case Toast -> mc.getToastManager().add(new MeteorToast.Builder(title).icon(Items.CHEST).text("Found Stash!").build());
                        case Both -> {
                            info("Found stash at (highlight)%s(default), (highlight)%s(default).", chunk.x, chunk.z);
                            mc.getToastManager().add(new MeteorToast.Builder(title).icon(Items.CHEST).text("Found Stash!").build());
                        }
                    }
                }
